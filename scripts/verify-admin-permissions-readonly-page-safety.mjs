...
for (const s of ["ADMIN_PERMISSION_DEFINITIONS","ADMIN_ROUTE_PERMISSIONS","allAdminPermissionKeys","enforcementEnabled: false","default-admin-allow-all"]) if (!api.includes(s)) failures.push("api missing "+s);
...
for (const s of ["allAdminPermissionKeys","permissions,","permissionsMode: \"default-admin-allow-all\"","permissionsEnforced: false"]) if (!session.includes(s)) failures.push("session missing "+s);
