...
assertOk(chunk.includes('border: "1px solid #00346e"') && chunk.includes('boxShadow: "0 28px 90px rgba(15, 23, 42, 0.34)"'), "Master info edit popup uses standard shell");
assertOk(!chunk.includes("top: 154") && !chunk.includes("translateX(-50%)"), "Master info edit popup no longer uses old fixed top shell");
assertOk(chunk.includes('background: "#00346e"') && chunk.includes('data-barsh-standard-modal-title="true"'), "Master info edit popup has navy centered header");
