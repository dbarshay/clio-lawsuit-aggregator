...
assertOk(chunk.includes('border: "1px solid #00346e"') && chunk.includes('boxShadow: "0 28px 90px rgba(15, 23, 42, 0.34)"'), "Payment popup uses standard shell");
assertOk(!chunk.includes("top: 104") && !chunk.includes("translateX(-50%)"), "Payment popup no longer uses old fixed top shell");
assertOk(chunk.includes('background: "#00346e"') && chunk.includes("Post Lawsuit Payment") && chunk.includes('color: "#ffffff"'), "Payment popup has navy centered header");
